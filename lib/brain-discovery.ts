...
interface DiscoveryResult { ip: string; port: number; }
...
async function discoverNeeoBrain(): Promise<DiscoveryResult | null> {
  return new Promise((resolve) => {
    ...
    setTimeout(() => { browser.stop(); resolve(null); }, 5000);
  });
}
...
export async function fetchNeeoBrainModel(adapter: AdapterInstance): Promise<NeeoBrainModel | null> {
  ...
  if (!ip || !port) {
    ...
    if (result) {
      ip = result.ip; port = String(result.port);
  ...
  try {
    ...
    for (const roomKey of Object.keys(data.rooms || {})) {
      ...
      const room: BrainRoom = { id: roomId, name: roomData.name, devices: [], recipes: [], scenarios: [] };
      ...
      if (hasDevices) {
        for (const devKey of Object.keys(roomData.devices)) {
          ...
          const localMacros: BrainMacro[] = Object.values(device.macros || {}).map((macro: any) => ({
            id: macro.key, name: macro.name, label: macro.label
          }));
      ...
      if (hasScenarios) {
        for (const scnKey of Object.keys(roomData.scenarios)) {
          ...
          const brainScenario: BrainScenario = { id: scenario.key, name: scenario.name, type: scenario.type };
    ...
    return { rooms, devices, recipes, scenarios, brainIp: ip!, brainPort: Number(port) };
  } catch (error) {
